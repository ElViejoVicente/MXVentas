...
{
    public partial class frmCatAdjustTypeList : Telerik.WinControls.UI.RadForm
    {
        private static frmCatAdjustTypeList m_FormDefInstance;
        public static frmCatAdjustTypeList DefInstance
        {
            get
            {
                if (m_FormDefInstance == null || m_FormDefInstance.IsDisposed)
                    m_FormDefInstance = new frmCatAdjustTypeList();
                return m_FormDefInstance;
            }
            set { m_FormDefInstance = value; }
        }
        public frmCatAdjustTypeList()
        {
            InitializeComponent();
        }

        private void frmCatAdjustTypeList_Load(object sender, EventArgs e)
        {
            //events
        ...
        protected void HeadersListView()
        {
            ...
            lvCatalog.Columns.Add("Tipo de Ajuste", 200, HorizontalAlignment.Left);
        ...
        protected void ListCatalog()
        {
            ...
            try
            {
                ...
                cnn.Open();
                OleDbCommand cmd = new OleDbCommand("SELECT ID_ADJUST_TYPE, ADJUST_TYPE, ENABLED FROM CAT_ADJUST_TYPE ORDER BY ADJUST_TYPE", cnn);
                OleDbDataReader dr = cmd.ExecuteReader();
                int i = 0;
                lvCatalog.Items.Clear();
                while (dr.Read())
                {
                    lvCatalog.Items.Add(dr["ID_ADJUST_TYPE"].ToString());
                    lvCatalog.Items[i].SubItems.Add(dr["ADJUST_TYPE"].ToString());
        ...
        protected void SelectToEditItem()
        {
            try
            {
                if (lvCatalog.SelectedItems.Count != 0)
                {
                    ...
                    frmCatAdjustTypeItem cat = new frmCatAdjustTypeItem(id);
        ...
        {
            frmCatAdjustTypeItem cat = new frmCatAdjustTypeItem(0);
            cat.StartPosition = FormStartPosition.CenterScreen;
            cat.ShowInTaskbar = false;
            cat.ShowDialog();
            if (cat.ACTION_SUCCESS)
