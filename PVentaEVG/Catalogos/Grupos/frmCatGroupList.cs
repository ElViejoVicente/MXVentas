...
{
    public partial class frmCatGroupList : Telerik.WinControls.UI.RadForm
    {
        private static frmCatGroupList m_FormDefInstance;
        public static frmCatGroupList DefInstance
        {
            get
            {
                if (m_FormDefInstance == null || m_FormDefInstance.IsDisposed)
                    m_FormDefInstance = new frmCatGroupList();
                return m_FormDefInstance;
            }
            set { m_FormDefInstance = value; }
        }
        public frmCatGroupList()
        {
            InitializeComponent();
        }

        private void frmCatGroupList_Load(object sender, EventArgs e)
        {
            //events
        ...
        protected void HeadersListView() {
            ...
            lvCatalog.Columns.Add("Grupo", 200, HorizontalAlignment.Left);
        ...
        protected void ListCatalog() {
            ...
            try
            {
                ...
                cnn.Open();
                OleDbCommand cmd = new OleDbCommand("SELECT ID_GRUPO, DESC_GRUPO, ENABLED FROM CAT_GRUPO ORDER BY DESC_GRUPO", cnn);
                OleDbDataReader dr = cmd.ExecuteReader();
                int i = 0;
                lvCatalog.Items.Clear();
                while (dr.Read()) {
                    lvCatalog.Items.Add(dr["ID_GRUPO"].ToString());
                    lvCatalog.Items[i].SubItems.Add(dr["DESC_GRUPO"].ToString());
        ...
        protected void SelectToEditItem() {
            try {
                if (lvCatalog.SelectedItems.Count != 0)
                {
                    ...
                    frmCatGroupItem cat = new frmCatGroupItem(id);
